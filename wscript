...
def options(ctx):
    ctx.load('compiler_cxx cxx11 cfitsio CCfits boost tinyhtm_cxx hires wcs json5_parser tablator')
...
def configure(ctx):
    ...
    ctx.load('compiler_cxx cxx11 cfitsio CCfits boost tinyhtm_cxx hires json5_parser tablator')
...
def build(ctx):
    ...
    ctx.program(
        ...
        use=['cxx11','cfitsio','CCfits','BOOST','hdf5','hdf5_cxx',
