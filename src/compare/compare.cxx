...
/// sky search.
///
/// curl -v -o 100-1a_091_result.hdf5
/// "http://hades.ipac.caltech.edu/TAP/sync?LANG=ADQL&REQUEST=doQuery&QUERY=SELECT+*+FROM+planck_toi_100_1a+WHERE+CONTAINS%28POINT%28%27J2000%27,ra,dec%29,CIRCLE%28%27J2000%27,0,0,179%29%29=1&format=hdf5"
///
  ...
  try
  {
    ...
    if (indexed_entries.size () != raw_entries.size ())
      throw std::runtime_error (
          "Sizes are different\n\traw: " + std::to_string (raw_entries.size ())
          + "\n\tindexed: " + std::to_string (indexed_entries.size ()));
    double max_error = 0;
