...
#define PUSH_ERR(func, minor, str)                                            \
  H5Epush (__FILE__, func, __LINE__, H5E_PLINE, minor, str)
...
#define H5PY_GET_FILTER(a, b, c, d, e, f, g)                                  \
  H5Pget_filter_by_id2 (a, b, c, d, e, f, g, NULL)
...
*/
    ...
    && (H5_VERS_RELEASE < 3 || !H5_USE_16_API)
...
size_t lzf_filter (unsigned flags, size_t cd_nelmts,
                   const unsigned cd_values[], size_t nbytes, size_t *buf_size,
                   void **buf);
...
int register_lzf (void)
{
  ...
  H5Z_class_t filter_class = {
    H5Z_CLASS_T_VERS,                      (H5Z_filter_t)(H5PY_FILTER_LZF),
    1,                                     1,
    "lzf",                                 NULL,
    (H5Z_set_local_func_t)(lzf_set_local), (H5Z_func_t)(lzf_filter)
  };
#else
  H5Z_class_t filter_class
      = {(H5Z_filter_t)(H5PY_FILTER_LZF), "lzf", NULL,
         (H5Z_set_local_func_t)(lzf_set_local), (H5Z_func_t)(lzf_filter) };
#endif
...
*/
herr_t lzf_set_local (hid_t dcpl, hid_t type, hid_t)
{
  ...
  r = H5PY_GET_FILTER (dcpl, H5PY_FILTER_LZF, &flags, &nelements, values, 0,
                       NULL);
  if (r < 0)
    return -1;
  ...
  if (!(flags & H5Z_FLAG_REVERSE))
    {
      ...
      */
      ...
      if (outbuf == NULL)
        {
          PUSH_ERR ("lzf_filter", H5E_CALLBACK,
            ...
            { /* compression failed */
              ...
              if (errno == E2BIG)
              ...
              else if (errno == EINVAL)
                {
                  ...
                  PUSH_ERR ("lzf_filter", H5E_CALLBACK,
                            "Invalid data for LZF decompression");
                  goto failed;
                }
              else
                {
                  PUSH_ERR ("lzf_filter", H5E_CALLBACK,
                            "Unknown LZF decompression error");
                  goto failed;
                }
