...
int main (int argc, char *argv[])
{
  ...
  try
  {
      ...
      for (int arg = 2; arg < argc; ++arg)
        {
          ...
          if (boost::iequals (path.extension ().string (), ".fits"))
                   ...
                   || boost::iequals (path.extension ().string (), ".hdf5"))
            {
              ...
              compound.insertMember ("signal",
                                     HOFFSET (planck_hdf5_entry, signal),
                                     H5::PredType::NATIVE_FLOAT);
    ...
    sort_and_index<planck_tod_entry>(data_file, scratch_file, htm_file, mem,
    ...
    for (int i = 0; i < nattr; i++)
      {
        att = group2.createAttribute (
            avec[i].getName (), avec[i].getDataType (), avec[i].getSpace ());
