...
tinyhtm::Spherical extract_position (const json5_parser::mValue &value)
{
  ...
  if (pos.size () != 2)
    throw std::runtime_error (
        "Wrong number of elements in an array.  Expected "
        "2 but got " + std::to_string (pos.size ()));
  if ((pos[0].type () != json5_parser::real_type && pos[0].type ()
                                                    != json5_parser::int_type)
  ...
  for (auto &v : json5.get_obj ())
    {
      if (v.first == "output")
      ...
      else if (v.first == "detector")
        {
          ...
          std::map<int, double> FWHM = { { 30, 32.239 },
                                         { 44, 27.005 },
                                         { 70, 13.252 },
                                         { 100, 9.66 },
                                         { 143, 7.27 },
      ...
      else if (v.first == "mcm_iterations")
        {
          if (v.second.type () != json5_parser::int_type)
            throw std::runtime_error (
                "Expected an integer for 'mcm_iterations'");
              ...
              if (element.first == "shape")
                {
                  if (v.second.type () != json5_parser::obj_type)
                    throw std::runtime_error (
                        "Expected an object for 'pos.shape'");
                  ...
                  if (type_iterator->second.type () != json5_parser::str_type)
                    throw std::runtime_error (
                        "Expected a string for 'pos.shape.type'");
                  ...
                  if (type == "circle")
                  ...
                  else if (type == "box")
                    {
                      ...
                      {
                        shape
                            = std::unique_ptr<tinyhtm::Box>(new tinyhtm::Box (
                                extract_position (s.at ("center")),
              ...
              else if (element.first == "frame")
                {
                  if (element.second.type () != json5_parser::str_type)
                    throw std::runtime_error (
                        "Expected a string for 'pos.frame'");
                  ...
                  if (boost::iequals ("galactic", cframe))
                    ...
                    {
                      throw std::runtime_error (
                          "Unknown value for pos.frame: " + cframe
                ...
                {
                  throw std::runtime_error (
                      "Unknown field in 'pos' structure: " + element.first);
                }
            }
        }
              ...
              if (element.second.type () == json5_parser::str_type)
                {
                  keywords.push_back (
                      { key,
                        { element.second.get_str (), std::string ("") } });
                }
              else if (element.second.type () == json5_parser::array_type)
                {
                  ...
                  if (a[0].type () != json5_parser::str_type
                    ...
                    throw std::runtime_error ("Expected a string or an array "
                                              "of strings for 'keywords." + key
                                              + "'");

                  keywords.push_back (std::make_pair (
                      key, std::make_pair (a[0].get_str (), a[1].get_str ())));
                }
              else
                {
                  throw std::runtime_error ("Expected a string or an array of "
                                            "strings for 'keywords." + key
                                            + "'");
                }
            }
        }
