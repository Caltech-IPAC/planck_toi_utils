...
hires::Sample get_sample_from_query (const std::string &data_file,
    ...
    if (tablator::Format (path).is_hdf5 ()
        && H5::H5File (input_file, H5F_ACC_RDONLY).getNumObjs () == 2)
      {
        ...
        samples.emplace_back (
            get_sample_from_query (input_file, *shape, projection));
      }
    else
      {
        samples
            = get_sample_from_table (path, columns, shape.operator bool(),
                                     coordinate_frame, keywords, center, size);
      }

    std::array<size_t, 2> nxy{
      { static_cast<size_t>(size.lon () / angResolution),
        static_cast<size_t>(size.lat () / angResolution) }
    };
    std::array<double, 2> crval{ { center.lon (), center.lat () } };

    constexpr double pi = boost::math::constants::pi<double>();
    hires::Hires hires (nxy, crval, angResolution * pi / 180, keywords,
                        samples);
    if (compute_minimap)
      {
